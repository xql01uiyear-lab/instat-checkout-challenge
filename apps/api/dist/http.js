...
export function matchesETag(header, current, weak = false, exists = true) {
    if (header.trim() === '*')
        return exists;
    ...
    while (remainder) {
        ...
        if (remainder.startsWith(',')) {
            if (++empty > 32)
                throw new DomainError(400, 'INVALID_PRECONDITION', 'Слишком много пустых элементов в заголовке.');
...
export function preconditions(request, exists = true, etag) {
...
export async function conditionalRead(request, reply, payload) {
