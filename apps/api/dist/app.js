...
const link = (href, method = 'GET') => ({
    href,
    method,
});
const idParams = (name) => Type.Object({ [name]: C.Id }, { additionalProperties: false });
const productParams = Type.Object({ productId: Type.String({ minLength: 1, maxLength: 100 }) }, { additionalProperties: false });
const metadataHeaders = { 'X-Request-Id': Type.String(), 'Cache-Control': Type.String() };
const isApiUrl = (url) => /^\/api(?:\/|$)/.test(url.split('?')[0]);
// OpenAPI 3.0 uses example; JSON Schema uses examples.
function openApiAnnotations(value) {
    if (Array.isArray(value))
        return value.map(openApiAnnotations);
    if (!value || typeof value !== 'object')
        return value;
    return Object.fromEntries(Object.entries(value).map(([key, item]) => key === 'examples' && Array.isArray(item)
        ? ['example', item[0]]
...
export async function buildApp(options = {}) {
    ...
    const allowedMethods = (url) => {
        const path = url.split('?')[0];
        const methods = ['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE', 'TRACE'];
    ...
    app.addHook('onRequest', async (request, reply) => {
        ...
        if (request.method === 'OPTIONS') {
            ...
            if (!allowed)
                throw new DomainError(404, 'ROUTE_NOT_FOUND', 'Маршрут не найден.');
            reply.header('Allow', allowed.join(', ')).header('Cache-Control', 'no-store');
            if (request.headers.origin)
                reply.header('Access-Control-Allow-Methods', allowed.join(', '));
        ...
        if (hasBody &&
            ...
            throw new DomainError(415, 'UNSUPPORTED_CONTENT_ENCODING', 'Сжатое тело запроса не поддерживается.');
        }
        if (['POST', 'PUT'].includes(request.method) &&
            hasBody &&
            request.headers['content-type']?.split(';')[0].trim().toLowerCase() !== 'application/json')
            throw new DomainError(415, 'UNSUPPORTED_MEDIA_TYPE', 'Передайте тело в формате application/json.');
    });
    app.addHook('onSend', async (request, reply, payload) => {
        if (/^\/api(?:\/|$)/.test(request.url.split('?')[0]) ||
            request.url.split('?')[0] === '/assets/demo.svg')
            return conditionalRead(request, reply, payload);
        return payload;
    });
    await app.register(swagger, {
        transform: ({ schema, url }) => ({ schema: openApiAnnotations(schema), url }),
            ...
            for (const path of Object.values(openapiObject.paths ?? {})) {
                ...
                if (path.get)
                    path.head = {
                        ...
                        responses: Object.fromEntries(Object.entries(path.get.responses).map(([status, response]) => {
                            const copy = { ...response };
                            if ('content' in copy)
                                delete copy.content;
                            return [status, copy];
        ...
        openapi: {
            ...
            info: {
                title: 'Checkout API',
                version: '1.0.0',
                description: 'POST /api/sessions с телом {} создаёт гостевую сессию. Вставьте data.token в Authorize без слова Bearer. Суммы указаны в копейках. Данные сохраняются после перезапуска. Оплата тестовая.',
            },
    ...
    app.setErrorHandler((error, request, reply) => {
                ...
                : status === 413
                    ...
                    : status === 415
                        ...
                        : status === 400
                            ? 'INVALID_REQUEST'
                            : 'INTERNAL_ERROR';
        const message = err instanceof DomainError
            ? err.message
            : status === 500
        ...
        if (status === 401)
            reply.header('WWW-Authenticate', code === 'SESSION_REQUIRED'
    ...
    const authorize = async (request) => {
        ...
        if (!match)
            throw new DomainError(401, 'SESSION_INVALID', 'Некорректный токен сессии.');
        store.session(match[1]);
        request.sessionToken = match[1];
    };
    function add(method, url, route, handler) {
        const errors = {
        ...
        const responses = Object.fromEntries((route.statuses ?? [200]).map((status) => [
            status,
            status === 204
                ? {
                    type: 'null',
                        ...
                        ...([201, 202].includes(status)
                            ? {
                                Location: Type.String({
                                    description: status === 202 ? 'Адрес проверки статуса.' : 'Адрес созданного ресурса.',
                                }),
                            }
                            : {}),
                        ...(status === 202
        ...
        app.route({
            ...
            schema: {
                ...
                headers: Type.Object({
                    ...
                    'if-none-match': Type.Optional(Type.String({ description: 'Совпадение: 304 для GET/HEAD, 412 для изменения.' })),
            ...
            preHandler: async (request) => {
                if (!['GET', 'HEAD'].includes(request.method)) {
                    ...
                    if (request.headers['if-match'] !== undefined ||
                        ...
                        preconditions(request, route.current?.(request) ?? true);
                }
            },
            handler,
        });
    }
    function respond(r, reply, data, links, status = 200) {
        if ([201, 202].includes(status))
            reply.header('Location', links.self.href);
        if (status === 202)
    ...
    }, (r, p) => respond(r, p, { name: 'Checkout API', version: '1.0.0' }, {
        self: link('/api'),
    ...
    }, (r, p) => respond(r, p, { cards: testCards, settlementDelayMs: store.paymentDelayMs }, { self: link('/api/sandbox') }));
    ...
    }, (r, p) => respond(r, p, store.sessionInfo(r.sessionToken, r.params.sessionId), {
        self: link(`/api/sessions/${r.params.sessionId}`),
        cart: link('/api/cart'),
        orders: link('/api/orders'),
    }));
    add('GET', '/api/cart', { tag: 'Cart', id: 'getCart', summary: 'Корзина', data: C.CartSchema }, (r, p) => {
        ...
        return respond(r, p, cart, {
            ...
            ...Object.fromEntries(cart.items.map((i) => [`item_${i.productId}`, link(`/api/cart/items/${i.productId}`)])),
        });
    });
    add('GET', '/api/cart/items/:productId', {
        tag: 'Cart',
        id: 'getCartItem',
    ...
    }, (r, p) => respond(r, p, store.item(r.sessionToken, r.params.productId), {
    ...
    add('PUT', '/api/cart/items/:productId', {
        tag: 'Cart',
        id: 'setCartItem',
        current: (r) => {
        ...
        const value = store.setItem(r.sessionToken, r.params.productId, r.body.quantity);
        return respond(r, p, value.item, { self: link(`/api/cart/items/${r.params.productId}`), cart: link('/api/cart') }, value.created ? 201 : 200);
    });
    add('DELETE', '/api/cart/items/:productId', {
        tag: 'Cart',
        id: 'removeCartItem',
        current: (r) => store.cart(r.sessionToken).items.some((i) => i.productId === r.params.productId),
    ...
    add('GET', '/api/checkout/options', {
        ...
        const cart = store.cart(r.sessionToken);
        return respond(r, p, { cart, deliveryMethods, paymentMethods }, {
    ...
    add('POST', '/api/orders', {
        ...
        const value = store.createOrder(r.sessionToken, r.body, r.headers['idempotency-key']);
    ...
    add('GET', '/api/orders/:orderId/payments', {
        tag: 'Payments',
        id: 'listPayments',
        ...
        return respond(r, p, values, {
            ...
            ...Object.fromEntries(values.map((v) => [`payment_${v.id}`, link(`/api/payments/${v.id}`)])),
        });
    });
    add('POST', '/api/orders/:orderId/payments', {
        tag: 'Payments',
        id: 'createPayment',
        ...
        const value = store.createPayment(r.sessionToken, r.params.orderId, r.headers['idempotency-key']);
        return respond(r, p, value.data, paymentLinks(value.data), value.created ? 201 : 200);
    });
    add('GET', '/api/payments/:paymentId', {
        tag: 'Payments',
        id: 'getPayment',
    ...
    add('POST', '/api/payments/:paymentId/simulations', {
        tag: 'Sandbox',
        id: 'createSimulation',
        ...
        const value = store.simulate(r.sessionToken, r.params.paymentId, r.body.scenario);
        return respond(r, p, value.data, simulationLinks(value.data), value.data.status === 'processing' ? 202 : value.created ? 201 : 200);
    });
    add('GET', '/api/payments/:paymentId/simulations/:simulationId', {
        tag: 'Sandbox',
        id: 'getSimulation',
