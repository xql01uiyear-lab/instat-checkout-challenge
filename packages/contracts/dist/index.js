...
const text = (description) => Type.String({ minLength: 1, maxLength: 500, description });
...
export const Links = Type.Record(Type.String(), object({
...
export const envelope = (data) => object({ data, meta: Meta, links: Links });
...
export const CheckoutOptionsSchema = object({
    cart: CartSchema,
    deliveryMethods: Type.Array(object({
        id: Type.Union([Type.Literal('pickup'), Type.Literal('courier')]),
        title: text(),
        price: money(),
        freeFrom: Type.Unsafe({ ...money(), nullable: true }),
